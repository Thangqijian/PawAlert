...
class _ReportEmergencyFormState extends State<ReportEmergencyForm> {
  ...
  final List<String> _allHelpTypes = [
    'Carrier',
    'Transport to Vet',
    'Medical Help',
    'Food',
    ...
    try {
      ...
      setState(() {
        ...
        _selectedState = _apiLocations.keys.firstWhere(
          (k) =>
              k.toLowerCase().contains(detState.toLowerCase()) ||
      ...
      if (_selectedState == null) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
            content: Text('Location found! Please select State manually.')));
      }
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
          content: Text('GPS Error. Ensure Emulator location is set.')));
    }
  }
  ...
  Future<void> _analyzeWithAI() async {
    if (_imageBytes == null || _descriptionController.text.length < 5) {
      ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Add photo and description first!')));
      return;
    }
    setState(() => _isAnalyzing = true);

    // AI now analyzes image + text to rank urgency
    final result = await _aiService.analyzeEmergencyWithImage(
        _imageBytes!, _descriptionController.text);
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
          content: Text('AI suggests: ${_status.toUpperCase()} urgency'),
  ...
  void _submitForm() async {
    ...
    // Combine standard help with custom 'Other' help
    List<String> finalHelpList = List.from(_selectedHelp);
    if (_isOtherSelected && _otherHelpController.text.isNotEmpty) {
      finalHelpList.add("Other: ${_otherHelpController.text.trim()}");
    }

    final tips = await _aiService.generateSafetyTips(
        _status, _descriptionController.text);
    ...
    final postData = {
      ...
      'helpNeeded': finalHelpList, // Combined list including "Other"
        ...
        child: ListView(
          ...
          children: [
            ...
            ElevatedButton.icon(
              ...
              label: Text(
                  _isAnalyzing ? 'Analyzing...' : 'Analyze Urgency with AI'),
              style: ElevatedButton.styleFrom(
                  backgroundColor: Colors.purple[50],
                  foregroundColor: Colors.purple),
            ),

            // Hybrid Urgency Selection UI
            const SizedBox(height: 20),
            const Text('Verify Urgency Level',
                style: TextStyle(fontWeight: FontWeight.bold)),
            const SizedBox(height: 10),
            Row(
              mainAxisAlignment: MainAxisAlignment.spaceEvenly,
              children: ['Moderate', 'Urgent', 'Critical'].map((level) {
                bool isSelected = _status == level.toLowerCase();
                return ChoiceChip(
                  label: Text(level,
                      style: TextStyle(
                          color: isSelected ? Colors.white : Colors.black)),
                  selected: isSelected,
                  selectedColor: _getUrgencyColor(level.toLowerCase()),
                  onSelected: (bool selected) {
                    if (selected) {
                      setState(() => _status = level.toLowerCase());
                    }
                  },
                );
              }).toList(),
